...
const { BN_S, BN_M, BN_L, BN_XL, name1Bytes32, getAddressFromEvent, infiniteApproval } = require("./constants");
...
describe("GameFactory", function () {
  ...
  beforeEach(async function () {
    ...
    await testL3P.approve(paymentManager.address, infiniteApproval);
  ...
  it("Should be possible to update all players in a batch", async function () {
    ...
    expect(stat1Player1.claimable).to.equal((parseInt(BN_L) + parseInt(BN_S)).toString()); // (L3P when added + update)
    ...
    expect(stat1Player2.claimable).to.equal((parseInt(BN_M) + parseInt(BN_S)).toString()); // (L3P when added + update)
    ...
    expect(stat2Player1.claimable).to.equal((parseInt(BN_L) + parseInt(BN_S)).toString()); // (L3P when added + update)
    ...
    expect(statPlayer3.claimable).to.equal((parseInt(BN_S) + parseInt(BN_S)).toString()); // (L3P when added + update)
  });
